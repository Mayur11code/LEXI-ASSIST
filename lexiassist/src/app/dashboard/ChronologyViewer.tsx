...
import { Loader2, Clock, CalendarDays, ShieldCheck, Quote, Network } from "lucide-react";
...
// A vibrant, cyber-legal color palette to cycle through for each event
const themeColors = [
  ...
  { text: "text-rose-500", border: "border-rose-500", borderHover: "hover:border-rose-500/50", bgHover: "hover:bg-rose-950/10", badgeBg: "bg-rose-950/30", badgeBorder: "border-rose-900/50", dot: "bg-rose-400", shadow: "shadow-[0_0_15px_rgba(244,63,94,0.3)]" },
  { text: "text-cyan-500", border: "border-cyan-500", borderHover: "hover:border-cyan-500/50", bgHover: "hover:bg-cyan-950/10", badgeBg: "bg-cyan-950/30", badgeBorder: "border-cyan-900/50", dot: "bg-cyan-400", shadow: "shadow-[0_0_15px_rgba(6,182,212,0.3)]" }
];
...
export default function ChronologyViewer({ activeCaseId }: { activeCaseId: string }) {
  ...
  const fetchChronology = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data && result.data.aiTimeline) {
        let rawTimeline = result.data.aiTimeline;
    ...
    <div className="flex flex-col h-full w-full bg-[#08080a] overflow-hidden rounded-2xl">
      ...
      <div className="border-b border-zinc-800/60 bg-[#0c0c0e] px-6 py-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0 shadow-sm z-10">
        <div>
          ...
          <p className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest pl-11">
            // Chronological extraction from verified case documents
          </p>
        </div>
        ...
        {timeline.length === 0 ? (
          <div className="flex flex-col h-full w-full items-center justify-center p-8 text-center border border-dashed border-zinc-800/60 rounded-2xl bg-[#0c0c0e]/30">
            ...
            <p className="text-zinc-500 text-sm max-w-md mb-6 leading-relaxed">
              The AI has not yet extracted a timeline for this case. Navigate to the AI Intake Chat and run a timeline extraction prompt.
            </p>
          </div>
        ) : (
          <div className="max-w-4xl mx-auto relative pt-4 pb-12">
            ...
            <AnimatePresence>
              {timeline.map((item, index) => {
                const dateText = item.eventDate || "Unknown Date";
                const descText = item.description || "Event description missing.";
                
                // Dynamically assign a color theme based on the index position
                ...
                return (
                  <motion.div 
                    ...
                    className="relative flex flex-col sm:flex-row items-start gap-8 sm:gap-14 mb-12 group"
                  >
                    
                    {/* Date Block (Desktop Left Side) */}
                    <div className="w-30 shrink-0 hidden sm:flex flex-col items-end text-right z-10 pt-2">
                      <span className={`text-sm font-bold ${theme.text} drop-shadow-md transition-colors`}>{dateText}</span>
                      <span className="text-[9px] font-mono uppercase tracking-widest mt-1 text-zinc-500 group-hover:text-zinc-400 transition-colors">
                        Event {index + 1}
                      </span>
                    </div>
                    ...
                    {/* Event Description Card (Right Side) */}
                    <div className="flex-1 pl-14 sm:pl-0 w-full z-10">
                      
                      {/* Mobile Date Override */}
                      ...
                      <div className={`p-6 rounded-2xl bg-zinc-900/30 border border-zinc-800/60 ${theme.borderHover} ${theme.bgHover} transition-all duration-300 shadow-xl group-hover:-translate-y-1`}>
                        ...
                        <div className="flex justify-between items-start mb-4">
                          ...
                          {item.verified && (
                            <span className={`flex items-center gap-1.5 text-[9px] font-mono ${theme.text} ${theme.badgeBg} px-2.5 py-1 rounded-md border ${theme.badgeBorder} uppercase tracking-wider`}>
                        ...
                        {/* Main Description */}
                        <p className="text-sm text-zinc-200 leading-relaxed font-medium mb-5">
                          {descText}
                        </p>
                        
                        {/* Verifiable Source Citation Block */}
                        {item.verifiableSourceCitation && (
                          <div className="bg-[#08080a] p-4 rounded-xl border border-zinc-800/80 relative overflow-hidden group-hover:border-zinc-700 transition-colors">
                            <div className="absolute left-0 top-0 bottom-0 w-1 bg-zinc-700/50" />
                            <div className="flex gap-3">
                              <Quote className={`w-4 h-4 ${theme.text} shrink-0 mt-0.5 opacity-80`} />
                              <div className="space-y-1">
                                <p className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest">
                                  Extracted Document Citation
                                </p>
