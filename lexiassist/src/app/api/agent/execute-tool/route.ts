...
export async function POST(req: Request) {
  ...
  try {
    ...
    const activeToolCalls =
      payload.toolCalls || (payload.toolCall ? [payload.toolCall] : null);
    ...
    const toolNames = activeToolCalls
      .map((t: any) => t.toolName)
      .join(', ');

    console.log(
      `[EXECUTE-TOOL] Executing Batch: [${toolNames}] | Session: ${sessionId}`
    );

    await pusher
      .trigger(`session-${sessionId}`, 'agent:progress', {
        step: `Executing: ${toolNames.split(',').join(' and ')}...`,
      ...
      try {
        switch (toolName) {
          case 'extractCaseChronology':
            toolExecutionResult = await executeExtractCaseChronology(
              extractCaseChronologySchema.parse(args)
            );
            break;

          case 'generatePreBriefRisk':
            toolExecutionResult = await executeGeneratePreBriefRisk(
              generatePreBriefRiskSchema.parse(args)
            );
            break;

          case 'generateDocumentRedlines':
            toolExecutionResult = await executeGenerateDocumentRedlines(
              generateDocumentRedlinesSchema.parse(args)
            );
            break;

          case 'matchVerifyLawyer':
            toolExecutionResult = await executeMatchVerifyLawyer(
              matchVerifyLawyerSchema.parse(args)
            );
            break;
    ...
    if (!publish.ok) {
      ...
      throw new Error(
        `Loop dispatch failed (${publish.status}): ${body}`
      );
    }

    console.log(
      `[EXECUTE-TOOL] Success: State returned to loop (Status ${publish.status})`
    );
    ...
    if (payload?.sessionId) {
      await pusher
        .trigger(`session-${payload.sessionId}`, 'agent:completed', {
